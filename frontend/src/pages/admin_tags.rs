...
pub fn AdminTagsPage() -> impl IntoView {
    ...
    let load_tags = Callback::new(move |_| {
        if let Some(token) = auth.token() {
            let tags_signal = tags;
            let error_signal = error_message;
            let loading_signal = is_loading;

            spawn_local(async move {
                loading_signal.set(true);
                match tags::list_tags(&token).await {
                    Ok(list) => tags_signal.set(list),
                    Err(err) => error_signal.set(format!("failed to load tags: {err}")),
                }
                loading_signal.set(false);
            });
        } else {
            error_message.set("sign in required to manage tags".to_string());
        }
    });
        ...
        Callback::new(move |ev: ev::SubmitEvent| {
            ...
            spawn_local(async move {
                let result = if let Some(id) = current_editing {
                    tags::update_tag(&token, &id, &payload).await.map(|_| ())
                } else {
                    tags::create_tag(&token, &payload).await.map(|_| ())
                };
                ...
                match result {
                    ...
                    Err(err) => error_signal.set(format!("failed to save tag: {err}")),
                }
        ...
        <BaseLayout>
            <div class="space-y-6">
                <header class="space-y-2">
                    <p class="text-xs uppercase tracking-[0.16em] text-cyan-300">"Admin / Tags"</p>
                    <h1 class="text-2xl font-semibold text-white">"Tag management"</h1>
                </header>
                ...
                <section class="space-y-3">
                                                            ...
                                                            <button
                                                                ...
                                                                on:click=move |_| {
                                                                    if let Some(token) = auth.token() {
                                                                        ...
                                                                        let load = load_tags;
                                                                        spawn_local(async move {
                                                                            if let Err(err) = tags::delete_tag(&token, &id).await {
                                                                                error_signal.set(format!("failed to delete tag: {err}"));
                                                                            } else {
                                                                                load.run(());
                                                                            }
                                                                        });
                    ...
                    >
                        <p class="text-sm text-slate-300">"Loading tags..."</p>
                    </Show>
                </section>
            </div>
        </BaseLayout>
